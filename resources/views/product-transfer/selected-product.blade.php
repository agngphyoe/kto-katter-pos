...
<input id="product_model" value="" hidden>
    <div class="w-64   relative border rounded-md shadow-xl   bg-white  overflow-hidden hover:scale-95 transition-all duration-300">
        ...
        <div class="px-3 py-4 ">
            <div class="flex flex-row items-center justify-between  mb-4 gap-3">
                @php
                    $quantity = \App\Models\LocationStock::where('location_id', $location_id)
                                                            ->where('product_id', $product->id)
                                                            ->value('quantity');
                @endphp
                <h1 class="text-xs font-jakarta text-noti font-semibold">qty - {{ $quantity }} /</h1>
                <input type="number" placeholder="Quantity"
                    class="w-28 px-4 text-xs py-1 rounded-full  outline-none outline outline-1 outline-paraColor" id="quantity{{$product->id}}" min="0" max="{{ $product->quantity }}" required>

                <button type="button" onclick="addToCard({{$product->id}})" class="pull">
