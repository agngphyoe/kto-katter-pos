@forelse($promotions as $promotion)
    <tr class="bg-white border-b text-left ">
        ...
        <td class="px-6 py-4 whitespace-nowrap">
            <button
                class="cursor-default outline-noti text-[12px] px-3 font-semibold rounded-full bg-primary text-white">
        ...
        <td class="px-6 py- whitespace-nowrap">
            <buton
                class="cursor-default outline-noti text-[12px] px-3 font-semibold rounded-full bg-gray-900 text-white">
        ...
        <td class="px-6 py-4 whitespace-nowrap text-center">
            @php
                $locations = explode(',', $promotion->locations);
            @endphp
            {{ count($locations) }}
        </td>
        <td class="px-6 py-4 whitespace-nowrap">
        ...
        <td class="px-6 py-4 whitespace-nowrap">
            {{ dateFormat($promotion->created_at) }}
        </td>

        <td class="px-6 py-4 whitespace-nowrap">
            <label class="toggle-switch">
                <input type="checkbox" id="check_status" name="status"
                    value="{{ $promotion->status == 'active' ? 'active' : 'inactive' }}"
                    @if ($promotion->status == 'active') checked @endif data-promotion-id="{{ $promotion->id }}"
        ...
        @if (!isset($is_export) || !$is_export)
            <td class="px-6 py-4">
                <div class="flex items-center gap-5 icon-container">
                    ...
                    <div class="flex items-center hover-icons gap-1">
                        <div class="flex flex-col gap-2">
                            @if (auth()->user()->hasPermissions('promotion-detail'))
                                <a href="{{ route('promotion-detail', ['promotion' => $promotion->id]) }}"
                            ...
                            @endif
                            {{-- @if (auth()->user()->hasPermissions('promotion-edit'))
                        <a href="{{ route('promotion-edit',['promotion'=>$promotion->id]) }}" class="bg-white outline outline-1 outline-primary h-8 w-8 flex items-center justify-center   rounded-full">
                    ...
                    @endif --}}
                            @if (auth()->user()->hasPermissions('promotion-delete'))
                                <a href="#" data-route="{{ route('promotion-delete', $promotion->id) }}"
                                    data-redirect-route="promotion"
