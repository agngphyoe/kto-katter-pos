@forelse($stockAdjustments as $stockAdjustment)
    <tr class="bg-white border-b text-left ">
        ...
        <td class="px-6 py-4 text-center whitespace-nowrap">
            {{ $stockAdjustment->stockAdjustmentProducts?->sum('quantity') ?? '-' }}
        </td>
        <td class="px-6 py-4  whitespace-nowrap">
        ...
        @if (!isset($is_export) || !$is_export)
            <td class="px-10 py-4 flex items-center justify-center">
                <div class="flex items-center justify-center  gap-5 icon-container">
                    ...
                    @if (auth()->user()->hasPermissions('product-adjustment-detail'))
                        <a href="{{ route('product-stock-detail', ['stockAdjustment' => $stockAdjustment->id]) }}">
                            <svg xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 576 512" class="mx-auto">
