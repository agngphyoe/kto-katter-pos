@forelse ($categories as $category)
    <tr class="bg-white text-left border-b  ">
        ...
        <td class="px-6 py-4 whitespace-nowrap">{{ dateFormat($category->created_at) }}</td>

        @if (!isset($is_export) || !$is_export)
            <td class="px-6 py-4 whitespace-nowrap flex justify-start items-center gap-4">
                {{-- edit --}}
                @if (auth()->user()->hasPermissions('category-edit'))
                ...
                {{-- delete --}}
                @if (auth()->user()->hasPermissions('category-delete'))
                    <a href="#" data-route="{{ route('category-delete', $category->id) }}"
                        data-redirect-route="category" class="deleteAction">
