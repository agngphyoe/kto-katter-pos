...
@forelse($products as $product)
    <tr class="bg-white border-b text-left ">
        @if (!isset($is_export) || !$is_export)
            <td scope="row" class="px-6 relative  py-4 whitespace-nowrap flex items-center gap-2   ">

                @if ($product->image)
                    <img src="{{ asset('products/image/' . $product->image) }}" class="w-10 object-contain h-10"
                        alt="x">
                @else
                    <img src="{{ asset('images/no-image.png') }}" class="w-10 object-contain" alt="">
                @endif

                <h1 class=" text-gray-900  "> {{ $product->code ?? '-' }}</h1>

                @if ($product->promotion_status)
                    <div class="absolute top-2 -right-5">

                        <div class="bg-noti px-1 rounded-lg text-white text-[9px]">Promo</div>
                    </div>
                @endif
            </td>
        @else
        ...
        <td class="py-4 whitespace-nowrap px-6 text-right" id="unit_price{{ $product->id }}">
        ...
        @if (!isset($is_export) || !$is_export)
            <td class="px-10 py-4 whitespace-nowrap ">
                <div class="flex items-center justify-center">
                    <div class="flex justify-center items-center  gap-5 icon-container">
                        ...
                        <div class="flex items-center hover-icons ">
                            <div class="flex flex-col gap-6">
                                ...
                                @if (auth()->user()->hasPermissions('product-detail'))
                                    <a
                                        href="{{ route('product-detail', ['product' => $product->id, 'page' => $products->currentPage()]) }}">
                                        <button
                            ...
                            @if (auth()->user()->hasPermissions('product-delete'))
                                <a href="#" data-route="{{ route('product-delete', $product->id) }}"
                                    data-redirect-route="product/list" class="deleteAction">
                                    <button
                                        class=" bg-white outline outline-1 outline-primary h-8 w-8 flex items-center justify-center  rounded-full">
