@forelse ($brands as $brand)
    <tr class="bg-white border-b text-left">
        ...
        <td class="px-6 py-4 whitespace-nowrap  ">{{ $brand->prefix ?? '-' }}</td>
        <td class="px-6 py-4 whitespace-nowrap  ">{{ $brand->category->name ?? '-' }}</td>
        <td class="px-6 py-4 whitespace-nowrap  ">{{ $brand->user->name ?? '-' }}</td>
        <td class="px-6 py-4 whitespace-nowrap">{{ dateFormat($brand->created_at) }}</td>

        @if (!isset($is_export) || !$is_export)
            <td class="px-6 py-4 flex justify-start items-center gap-4">
                {{-- edit --}}
                @if (auth()->user()->hasPermissions('brand-edit'))
                ...
                {{-- delete --}}
                @if (auth()->user()->hasPermissions('brand-delete'))
                    <a href="#" data-route="{{ route('brand-delete', $brand->id) }}" data-redirect-route="brand"
                        class="deleteAction">
