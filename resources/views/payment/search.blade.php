...
<tr class="bg-white border-b text-left ">
    <th scope="row" class="px-6 whitespace-nowrap py-4  font-medium  text-gray-900 flex items-center gap-2 whitespace-nowrap ">


        @if($payment->paymentableBy?->image)
        <img src="{{asset('customers/image/'.$payment->paymentableBy?->image)}}" class="w-10 h-10 object-contain " alt="">
        @else
        <img src="{{ asset('images/no-image.png') }}" class="w-10 h-10">
        @endif

        <h1>{{ $payment->paymentableBy?->name }}( {{ $payment->paymentableBy?->user_number}} )</h1>
    ...
    <td class=" py-4 px-6 text-right whitespace-nowrap text-noti ">
    ...
    <td class=" py-4 px-6 text-right text-noti ">
    ...
    <td class=" py-4 px-6 whitespace-nowrap">
        <div class="flex items-center gap-3">
            <div class="bg-gray-200 w-20 h-1 rounded-full">
                <div class="h-1 bg-primary  text-[6px] text-center  rounded-full text-white " style="width: {{ round(calculatePaymentProgress($payment)[0]) }}%"></div>
            </div>
            <h1 class="text-primary">{{ round(calculatePaymentProgress($payment)[0])}}%</h1>
        </div>
    </td>
                ...
                <div class="flex items-center gap-5 icon-container">
                    <button class="bg-bgMain h-9 w-9 flex items-center justify-center rounded-full">
                        <svg xmlns="http://www.w3.org/2000/svg" height="1.5em" viewBox="0 0 128 512">
                            <path d="M64 360a56 56 0 1 0 0 112 56 56 0 1 0 0-112zm0-160a56 56 0 1 0 0 112 56 56 0 1 0 0-112zM120 96A56 56 0 1 0 8 96a56 56 0 1 0 112 0z" fill="#00812C" class="centered-svg" />
                        </svg>
                    </button>
                    <div>
                    ...
                    <div class="flex items-center hover-icons ">
                        <div class="flex flex-col gap-6">
                            {{-- detail --}}
                            @if(auth()->user()->hasPermissions('payment-detail'))
                            <a href="{{ route('payment-detail',['payment'=>$payment->id]) }}" class="bg-white outline outline-1 outline-primary h-8 w-8 flex items-center justify-center rounded-full">
