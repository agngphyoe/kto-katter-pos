@forelse ($locations as $location)
    <tr class="bg-white border-b text-left">
        ...
        <td class="px-6 py-4 text-noti text-center">{{ $location->total_products ?? '-' }}</td>
        <td class="px-6 py-4 text-noti text-center">{{ $location->total_quantities ?? '-' }}</td>
        <td class="px-6 py-4 text-noti text-right">{{ number_format($location->total_retail_selling_price) ?? '-' }}</td>
