@forelse ($types as $type)
    <tr class="bg-white border-b  text-left ">
        ...
        <td class="px-6 py-4 whitespace-nowrap">{{ $type->user?->name ?? '-' }}</td>
        <td class="px-6 py-4 whitespace-nowrap">{{ dateFormat($type->created_at) }}</td>

        @if (!isset($is_export) || !$is_export)
            <td class="px-6 py-4 whitespace-nowrap flex justify-start items-center  gap-4">
                {{-- edit --}}
                @if (auth()->user()->hasPermissions('type-edit'))
                ...
                {{-- delete --}}
                @if (auth()->user()->hasPermissions('type-delete'))
                    <a href="#" data-route="{{ route('type-delete', $type->id) }}" data-redirect-route="type"
                        class="deleteAction">
