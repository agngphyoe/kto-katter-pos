@forelse ($pls as $pl)
    <tr class="border-b text-left">
        <td class="px-6 py-4 whitespace-nowrap">{{ $pl->profit_and_loss_number }}</td>
        <td class="px-6 py-4 whitespace-nowrap">{{ $pl->month }}</td>
        <td class="px-6 py-4 whitespace-nowrap">{{ $pl->user->name ?? '-' }}</td>
        <td class="py-4 px-10">
                ...
                <div class="flex items-center gap-5 icon-container">
                    ...
                    <div class="flex items-center hover-icons">
                        <div class="flex flex-col gap-6">
                            {{-- Detail button --}}
                            <a href="{{ route('profit-and-loss-details', ['id' => $pl->id]) }}"
