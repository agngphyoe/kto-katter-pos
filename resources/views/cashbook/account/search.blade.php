@forelse ($accounts as $account)
    <tr class="bg-white border-b text-left">
        ...
        <td class="px-6 py-4 whitespace-nowrap  ">{{ $account->accountType->name }}</td>
        <td class="px-6 py-4 whitespace-nowrap  ">{{ dateFormat($account->created_at) }}</td>

        <td class="px-10 py-4 whitespace-nowrap flex justify-center items-center gap-4">
            {{-- edit --}}
            @if (auth()->user()->hasPermissions('account-edit'))
            ...
            @endif
            @if (auth()->user()->hasPermissions('account-delete'))
                @if (!$account->hasAnyRelations())
                    {{-- delete --}}
                    <a href="#" data-route="{{ route('account-delete', $account->id) }}"
                        data-redirect-route="cashbook/settings/account" class="deleteAction">
