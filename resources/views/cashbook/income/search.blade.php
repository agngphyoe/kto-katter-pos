@forelse ($incomes as $income)
    <tr class="bg-white border-b text-left">
        ...
        <td class="px-6 py-4 whitespace-nowrap text-right ">{{ number_format($income->amount) }}</td>
        ...
        <td class="px-6 py-4 whitespace-nowrap">{{ dateFormat($income->issue_date) }}</td>
        ...
        <td class="px-10 py-4 whitespace-nowrap flex justify-center items-center gap-4">
            {{-- edit --}}
            @if (auth()->user()->hasPermissions('income-edit'))
            ...
            @endif
            {{-- delete --}}
            @if (auth()->user()->hasPermissions('income-delete'))
                <a href="#" data-route="{{ route('income-delete', $income->id) }}"
                    data-redirect-route="cashbook/income" class="deleteAction">
