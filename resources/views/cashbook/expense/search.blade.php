@forelse ($expenses as $expense)
    <tr class="bg-white border-b text-left">
        ...
        <td class="px-6 py-4 whitespace-nowrap text-right ">{{ number_format($expense->amount) }}</td>
        ...
        <td class="px-6 py-4 whitespace-nowrap">{{ dateFormat($expense->issue_date) }}</td>
        ...
        <td class="px-10 py-4 whitespace-nowrap flex justify-center items-center gap-4">
            {{-- edit --}}
            @if (auth()->user()->hasPermissions('expense-edit'))
            ...
            @endif
            {{-- delete --}}
            @if (auth()->user()->hasPermissions('expense-delete'))
                <a href="#" data-route="{{ route('expense-delete', $expense->id) }}"
                    data-redirect-route="cashbook/expense" class="deleteAction">
