@forelse ($others as $other)
    <tr class="bg-white border-b text-left">
        ...
        <td class="px-6 py-4 whitespace-nowrap text-right ">{{ number_format($other->amount) }}</td>
        ...
        <td class="px-6 py-4 whitespace-nowrap  ">{{ $other->transaction_type }}</td>
        <td class="px-6 py-4 whitespace-nowrap  ">{{ dateFormat($other->issue_date) }}</td>

        <td class="px-10 py-4 whitespace-nowrap flex justify-center items-center gap-4">
            {{-- edit --}}
            @if (auth()->user()->hasPermissions('other-assets-edit'))
                <a href="{{ route('others-edit', ['other' => $other->id]) }}">
            ...
            @endif
            {{-- delete --}}
            @if (auth()->user()->hasPermissions('other-assets-delete'))
                <a href="#" data-route="{{ route('others-delete', $other->id) }}"
                    data-redirect-route="cashbook/others" class="deleteAction">
