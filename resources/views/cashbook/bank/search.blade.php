@forelse ($banks as $bank)
    <tr class="bg-white border-b text-left">
        ...
        <td class="px-6 py-4 whitespace-nowrap ">{{ $bank->user?->name }}</td>
        <td class="px-6 py-4 whitespace-nowrap ">{{ dateFormat($bank->created_at) }}</td>

        <td class="px-10 py-4 whitespace-nowrap flex justify-center items-center gap-4">
            {{-- edit --}}
            @if (auth()->user()->hasPermissions('bank-edit'))
            ...
            @endif
            {{-- delete --}}
            @if (auth()->user()->hasPermissions('bank-delete'))
                @if (!$bank->hasAnyRelations())
                    <a href="#" data-route="{{ route('bank-delete', $bank->id) }}"
                        data-redirect-route="cashbook/settings/bank" class="deleteAction">
