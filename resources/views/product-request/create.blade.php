...
@endphp
<section class="product_request_create">
    {{-- nav start  --}}
    @include('layouts.header-section', [
    'title' => 'Create A New Product Request',
    'subTitle' => 'Fill to build a new Request',
    ])
    {{-- nav end  --}}
        ...
        <div class="lg:flex  lg:justify-around gap-10">
            ...
            <div class="bg-white w-full md:w-[800px] mx-auto lg:mx-0 mb-5 lg:mb-0  rounded-[20px]">
                <div class="px-5 md:px-14 py-10 ">
                    ...
                    <form action="{{ route('product-request-create-second') }}" enctype="multipart/form-data">
                        @csrf
                        <div>
                            {{-- product code  --}}
                            @php
                            ...
                            <div class="mb-10 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                                <label for="" class="font-jakarta text-paraColor font-semibold">Request ID </label>
                                <div class="mr-[50px]">
                                    <div class="outline outline-1 flex items-center text-sm font-jakarta text-paraColor w-[300px] outline-primary rounded-full ">
                                        <div class=" py-2 px-3 bg-bgMain text-primary border-r border-primary rounded-l-full">
                                            {{ PrefixCodeID::PO_REQUEST }} -
                                        </div>
                                        <input type="number" placeholder="Enter Product Code" name="request_code" value="{{ $product_code[1] }}" class=" py-2 flex-grow rounded-r-full  px-3  outline-none">
                                    </div>
                                    @error('code')
                            ...
                            <div class="mb-10 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                                <label for="" class="font-jakarta text-paraColor font-semibold">Request From *</label>
                                ...
                                <div class="mr-[50px]">
                                    <select name="from_location_id" class="product-select select2" id="from_location_id_select" required>
                                        @forelse($from_locations as $from)
                                            <option value="{{ $from->location_id }}" class="outline-none font-Pop">{{ $from->location_name }}</option>
                                        @empty
                            ...
                            <div class="mb-10 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                                <label for="" class="font-jakarta text-paraColor font-semibold">Request To *</label>
