@forelse ($sale_consultants as $data)
    <tr class="bg-white border-b text-left">
        <td class="px-6 py-4">{{ $data->name ?? '-' }}</td>
        <td class="px-6 py-4">{{ $data->location->location_name }}</td>
        @if (!isset($is_export) || !$is_export)
            <td class="px-6 py-4 flex justify-start items-center gap-4">
                {{-- edit --}}
                <a href="{{ route('sc-edit', ['id' => $data->id]) }}">
                ...
                {{-- delete --}}
                <a href="#" data-route="{{ route('sc-delete', ['id' => $data->id]) }}"
                    data-redirect-route="sale-consultants" class="deleteAction">
                ...
                <a href="{{ route('sc-details', ['id' => $data->id]) }}">
