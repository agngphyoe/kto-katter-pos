...
@section('content')
    <section class="product-model-create">
        ...
        @include('layouts.header-section', [
            'title' => 'Edit A Sale Staff',
            'subTitle' => 'Fill to edit a sale staff',
        ])
        {{-- nav end  --}}


        <form id="myForm" action="{{ route('sc-update', ['id' => $data->id]) }}" method="POST">
            @csrf
            {{-- box start  --}}
                        ...
                        <div class="flex items-center justify-center gap-10">
                            ...
                            <div>
                                ...
                                <select name="location_id" class="select2 w-[220px]" required>
                                    ...
                                    <option value="{{ $location->id }}" @if($location->id == $data->location_id) selected @endif>{{ $location->location_name }}</option>
                                    @endforeach
                                </select>
                                @error('location_id')
                            ...
                            <div>
                                ...
                                <input type="text" name="name" id="brand_input" value="{{ $data->name }}"
