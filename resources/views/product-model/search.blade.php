@forelse ($product_models as $product_model)
    <tr class="bg-white text-left border-b  ">
        <td class="px-6 py-4 whitespace-nowrap">{{ $product_model->name }}</td>
        <td class="px-6 py-4 whitespace-nowrap">{{ $product_model->brand?->name ?? '-' }}</td>
        <td class="px-6 py-4 whitespace-nowrap">{{ $product_model->brand?->category?->name }}</td>
        <td class="px-6 py-4 whitespace-nowrap">{{ $product_model->user?->name ?? '-' }}</td>
        <td class="px-6 py-4 whitespace-nowrap">{{ dateFormat($product_model->created_at) }}</td>

        @if (!isset($is_export) || !$is_export)
            <td class="px-6 py-4 whitespace-nowrap flex justify-start items-center gap-4">
                @if (auth()->user()->hasPermissions('model-edit'))
                    <a href="{{ route('product-model-edit', ['product_model' => $product_model->id]) }}">
                ...
                {{-- delete --}}
                @if (auth()->user()->hasPermissions('model-delete'))
                    <a href="#" data-route="{{ route('product-model-delete', $product_model->id) }}"
                        data-redirect-route="product-model" class="deleteAction">
