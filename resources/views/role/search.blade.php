@forelse ($roles as $role)
    <tr class="bg-white border-b text-left  ">
        ...
        <td class="px-6 py-4 whitespace-nowrap text-center" style="color: rgb(130, 202, 178)">
            {{ $role->permissions?->count() }}
        </td>
        <td class="px-6 py-4 whitespace-nowrap text-center" style="color: rgb(244, 38, 48)">
            {{ $permission_count }}
        </td>
        <td class="px-6 py-4 whitespace-nowrap">{{ dateFormat($role->created_at) ?? '-' }}</td>
        <td class="px-6 py-4 whitespace-nowrap">{{ $role->user?->name ?? '-' }}</td>
        @if (!isset($is_export) || !$is_export)
            <td class="px-10 py-4 flex justify-start items-center  gap-4">
                ...
                @if (!$role->hasAlreadyUse())
                    {{-- delete --}}
                    <a href="#" data-route="{{ route('role-delete', ['role' => $role->id]) }}"
                        data-redirect-route="role" class="deleteAction">
