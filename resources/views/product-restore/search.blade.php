@forelse($product_restores as $restore)
    <tr class="bg-white border-b text-left ">
        <td scope="row" class=" py-4 whitespace-nowrap px-6      ">
            {{ $restore->return_inv_code ?? '' }}
        </td>
        <td scope="row" class=" py-4 whitespace-nowrap px-6      ">
            {{ $restore->remark ?? '-' }}
        </td>
        <td class=" py-4 whitespace-nowrap px-6">
            {{ $restore->fromLocationName->location_name ?? '-' }}
        </td>
        <td class=" py-4 whitespace-nowrap px-6">
            {{ $restore->toLocationName->location_name ?? '-' }}
        </td>
        <td class=" py-4 whitespace-nowrap px-6">
            {{ $restore->user->name ?? '-' }}
        </td>
        ...
        <td class=" py-4 whitespace-nowrap px-6 w-48">
            {{ dateFormat(Carbon\Carbon::parse($restore->created_at)->format('d-m-Y')) }}
        </td>
        ...
        @if (!isset($is_export) || !$is_export)
            <td class=" py-4 whitespace-nowrap  ">
                <div class="flex items-center justify-center">
                    <div class="flex items-center  gap-5 icon-container">
                        ...
                        <div class="flex items-center hover-icons ">
                            <div class="flex flex-col gap-6">
                                ...
                                {{-- detail --}}
                                @if (auth()->user()->hasPermissions('restore-detail'))
                                    <a
                                        href="{{ route('product-restore-detail', ['product' => $restore->return_inv_code]) }}">
                                        <button
                        ...
                        <a href="{{ route('product-edit', ['product' => $restore->id]) }}">
                    ...
                    @if (!$restore->productables()->exists())
                    <a href="#" data-route="{{ route('product-delete', $restore->id) }}" data-redirect-route="product/list" class="deleteAction">
                        <button class=" bg-white outline outline-1 outline-primary h-8 w-8 flex items-center justify-center  rounded-full">
