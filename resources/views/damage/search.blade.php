@forelse($damages as $damage)
    <tr class="bg-white border-b text-left ">
        ...
        @if (!isset($is_export) || !$is_export)
            <td class="px-10 py-4 whitespace-nowrap flex justify-center items-center">
                <div class="flex items-center gap-5 icon-container">
                    <button class="bg-bgMain h-9 w-9 flex items-center justify-center rounded-full">
                        <svg xmlns="http://www.w3.org/2000/svg" height="1.5em" viewBox="0 0 128 512" class="mx-auto">
                            <path
                    ...
                    <div class="flex items-center hover-icons ">
                        <div class="flex flex-col gap-6">
                            ...
                            {{-- damage --}}
                            @if (auth()->user()->hasPermissions('product-damage-detail'))
                                <a href="{{ route('damage-detail', ['damage' => $damage->id]) }}"
                            ...
                            {{-- edit --}}
                            {{-- @if (auth()->user()->hasPermissions('product-damage-edit'))
                    <a href="{{ route('damage-edit-first',['id'=>$damage->id]) }}" class="bg-white outline outline-1 outline-primary h-8 w-8 flex items-center justify-center   rounded-full">
                    ...
                    @endif --}}
                        ...
                        @if (auth()->user()->hasPermissions('product-damage-delete'))
                            {{-- <a href="#" data-route="{{ route('damage-delete', $damage->id) }}" data-redirect-route="damage" class="deleteAction bg-white outline outline-1 outline-primary h-8 w-8 flex items-center justify-center  rounded-full">
