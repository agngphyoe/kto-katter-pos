@forelse($transfers as $transfer)
    <tr class="bg-white border-b text-left ">
        ...
        <td scope="row" class="py-4 whitespace-nowrap px-6">
            @php
                $purchase_number = \App\Models\Purchase::where('id', $transfer->purchase_id)->value('invoice_number');
            @endphp
            {{ $purchase_number }}
        </td>
        <td scope="row" class="py-4 whitespace-nowrap px-6">
            {{ $transfer->location->location_name }}
        </td>
        <td scope="row" class="py-4 whitespace-nowrap px-6 text-center">
            {{ $transfer->quantity }}
        </td>
        <td scope="row" class="py-4 whitespace-nowrap px-6">
            {{ dateFormat($transfer->added_date) }}
        </td>
            ...
            <div class="flex items-center  gap-5 icon-container">
                ...
                <div class="flex items-center hover-icons ">
                    <div class="flex flex-col gap-6">
                        <a href="{{ route('product-purchase-stock-history-details', ['id' => $transfer->id]) }}">
