@forelse($poTransfers as $transfer)
<tr class="bg-white border-b text-left ">
    <td scope="row" class=" py-4 whitespace-nowrap px-6      ">
        {{ $transfer->request_inv_code ?? '' }}
    </td>
    <td scope="row" class=" py-4 whitespace-nowrap px-6      ">
        {{ $transfer->remark ?? '-' }}
    </td>
    <td class=" py-4 whitespace-nowrap px-6">
        {{ $transfer->fromLocationName->location_name ?? '-' }}
    </td>
    <td class=" py-4 whitespace-nowrap px-6">
        {{ $transfer->toLocationName->location_name ?? '-' }}
    </td>
    <td class=" py-4 whitespace-nowrap px-6">
        {{ $transfer->user->name ?? '-' }}
    </td>
    ...
    <td class=" py-4 whitespace-nowrap px-6 w-48">
        {{ dateFormat(Carbon\Carbon::parse($transfer->created_at)->format('d-m-Y')) }}
    </td>
            ...
            <div class="flex items-center  gap-5 icon-container">
                ...
                <div class="flex items-center hover-icons ">
                    <div class="flex flex-col gap-6">
                        ...
                        <a href="{{ route('po-transfer-detail', ['product' => $transfer->request_inv_code]) }}">
                    ...
                    @if (!$transfer->productables()->exists())
                    <a href="#" data-route="{{ route('product-delete', $transfer->id) }}" data-redirect-route="product/list" class="deleteAction">
                        <button class=" bg-white outline outline-1 outline-primary h-8 w-8 flex items-center justify-center  rounded-full">
