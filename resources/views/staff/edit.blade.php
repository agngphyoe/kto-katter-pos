            ...
            <form action="{{ route('staff-update', $staff->id) }}" method="POST">
                ...
                <div class="grid grid-cols-1   lg:grid-cols-5 mx-4 sm:mx-10 gap-3">
                    <div class="col-span-1 lg:col-span-2 ">
                        <img src="{{ asset('images/createCustomer.png') }}" class="w-full lg:mt-20" alt="">
                        ...
                        <form action="{{ route('customer-store') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="flex items-center  justify-center flex-wrap gap-5">
                                ...
                                <!-- <div class="">
                                    ...
                                    <div class="">
                                        <select id="myselect5" name="division_id"
                                            class="outline-none  text-sm pr-2 text-paraColor w-[300px]  font-medium  px-4 myselect-5 select2">
                                            <option value="" disabled selected>Division</option>
                                            @forelse($divisions as $division)
                                                <option value="{{ $division->id }}"
                                                    {{ $staff->division_id == $division->id ? 'selected' : '' }}>
                                ...
                                <!-- <div id="townshipSelectContainer">
                                        ...
                                        <label for=""
                                            class=" block mb-2 text-paraColor font-semibold  text-sm">Township</label>
                                        <div class="">
                                            <select id="myselect6" name="township_id"
                                                class="outline-none  text-sm pr-2 text-paraColor w-[300px]  font-medium  px-4 myselect-5 select2">
                                                <option value="" disabled selected>Township</option>
                                                @forelse($townships as $township)
                                                    <option value="{{ $township->id }}"
                                                        {{ $staff->township_id == $township->id ? 'selected' : '' }}>
                                ...
                                {{-- password --}}
                                <div>
                                    <label for="" class="block mb-2 text-sm text-paraColor font-semibold ">Password</label>
                            ...
                            <div
                                ...
                                <a href="{{ route('staff') }}"
                                    class="outline outline-1 outline-noti w-full sm:w-80 mb-2 rounded-full py-2  text-noti text-center">Cancel</a>
                                <button type="submit"
                                    class="outline outline-1 outline-primary w-full sm:w-80 mb-4 rounded-full py-2 text-white bg-primary"
                                    id="done">Done</button>
                            </div>
