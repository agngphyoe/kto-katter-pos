@forelse ($locations as $location)
    <tr class="bg-white border-b text-left">
        ...
        <td class="px-6 py-4 whitespace-nowrap">{{ $location->locationType?->location_type_name ?? '-' }}</td>
        ...
        @if (!isset($is_export) || !$is_export)
            <td class="px-10 py-4 flex justify-start items-center gap-4">
                ...
                {{-- delete --}}
                @php
                    $stockExist = \App\Models\LocationStock::where('location_id', $location->id)->sum('quantity');

                @endphp
                @if ($stockExist == 0)
                    <a href="#" data-route="{{ route('location-destroy', ['location' => $location->id]) }}"
                        data-redirect-route="location" class="deleteAction">
